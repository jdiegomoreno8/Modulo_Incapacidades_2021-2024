...
export class PerfilComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    Object.keys(this.datosPerfil).map((key) => { this.listaEntidades.push(this.datosPerfil[key].nombreEntidad);})
  ...
  seleccionarEntidad(e) {

    this.listaRoles = [];
    this.Entidad = (e.target.value);
    const datosEntidad = this.datosPerfil.filter(s => s.nombreEntidad.includes(this.Entidad));
  ...
  seleccionarRol(e) {
    ...
    this.datosFalt = this.datosPerfil.filter(s => (s.nombreRol?.includes(this.Rol) && s.nombreEntidad.includes(this.Entidad)));
    
    this.obtenerFuncionesService.TraerIDUsuarioEntidad(this.datosFalt[0])
      .subscribe(data => {
